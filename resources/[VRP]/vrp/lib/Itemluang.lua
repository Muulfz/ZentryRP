...
-- Itemlang object methods

-- load a dict table to the itemlang dict
function Itemlang:load(dict)
  if dict then
    Itemluang.inject(self.dict, dict)
  end
end

-- load a dict table to the itemlang dict for a specific locale
function Itemlang:loadLocale(locale, dict)
  self:load({[locale] = dict})
end

-- construct Itemlang object
setmetatable(Itemluang, { __call = function(t)
  local obj = {}
  obj.dict = {}
  obj.itemlang = setmetatable({}, { __index = function(t,k) return resolve_path(obj.dict,"",t,k) end })
  return setmetatable(obj, { __index = Itemlang })
end})

-- inject recursively the itable (insert table) properties into the btable (base table)
function Itemluang.inject(btable, itable)
  if type(itable) == "table" then
    for k,v in pairs(itable) do
      local bv = btable[k]
      if type(bv) == "table" then
        Itemluang.inject(bv, v) -- recursive, don't replace table
      else
