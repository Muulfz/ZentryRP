...
AddEventHandler("vRP:playerSpawn",function(user_id, source, first_spawn)
    if first_spawn then
        ...
        for k,v in pairs(tps) do
            if v.active then
                vRP.setTpIn(source,k,v.pos_in.x,v.pos_in.y,v.pos_in.z,v.pos_in.h,v.pos_out.x,v.pos_out.y,v.pos_out.z,v.pos_out.h)
                vRP.setTpOut(source,k,v.pos_in.x,v.pos_in.y,v.pos_in.z,v.pos_in.h,v.pos_out.x,v.pos_out.y,v.pos_out.z,v.pos_out.h)
            end
        end
    end
end)

function vRP.setTpIn(user,name,pos_in_x,pos_in_y,pos_in_z,pos_in_h,pos_out_x,pos_out_y,pos_out_z,pos_out_h)
    Citizen.CreateThread(function()
        vRP.setArea(user,"vRP:cmd:tp:in:"..name,pos_in_x,pos_in_y,pos_in_z,1,1.5,function(player, area)
            vRPclient.cmdTeleport(player,pos_out_x,pos_out_y,pos_out_z,pos_out_h)
            vRPclient.removeNamedMarker(player,"vRP:cmd:tp:out:"..name)
            vRP.removeArea(player,"vRP:cmd:tp:out:"..name)
            SetTimeout(5000,function()
                vRP.setTpOut(player,name,pos_in_x,pos_in_y,pos_in_z,pos_in_h,pos_out_x,pos_out_y,pos_out_z,pos_out_h)
            end)
        end,nil)
...
function vRP.setTpOut(user,name,pos_in_x,pos_in_y,pos_in_z,pos_in_h,pos_out_x,pos_out_y,pos_out_z,pos_out_h)
    Citizen.CreateThread(function()
        vRP.setArea(user,"vRP:cmd:tp:out:"..name,pos_out_x,pos_out_y,pos_out_z,1,1.5,function(player, area)
            vRPclient.cmdTeleport(player,pos_in_x,pos_in_y,pos_in_z,pos_in_h)
            vRPclient.removeNamedMarker(player,"vRP:cmd:tp:in:"..name)
            vRP.removeArea(player,"vRP:cmd:tp:in:"..name)
            SetTimeout(5000,function()
                vRP.setTpIn(player,name,pos_in_x,pos_in_y,pos_in_z,pos_in_h,pos_out_x,pos_out_y,pos_out_z,pos_out_h)
            end)
        end,nil)
