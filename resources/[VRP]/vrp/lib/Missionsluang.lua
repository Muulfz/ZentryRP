...
-- Missionslang object methods

-- load a dict table to the missionslang dict
function Missionslang:load(dict)
    if dict then
        Missionsluang.inject(self.dict, dict)
    end
end

-- load a dict table to the missionslang dict for a specific locale
function Missionslang:loadLocale(locale, dict)
    self:load({[locale] = dict})
end

-- construct Missionslang object
setmetatable(Missionsluang, { __call = function(t)
    local obj = {}
    obj.dict = {}
    obj.missionslang = setmetatable({}, { __index = function(t,k) return resolve_path(obj.dict,"",t,k) end })
    return setmetatable(obj, { __index = Missionslang })
end})

-- inject recursively the itable (insert table) properties into the btable (base table)
function Missionsluang.inject(btable, itable)
    if type(itable) == "table" then
        for k,v in pairs(itable) do
            local bv = btable[k]
            if type(bv) == "table" then
                Missionsluang.inject(bv, v) -- recursive, don't replace table
            else
