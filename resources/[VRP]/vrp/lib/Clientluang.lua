...
-- Clientlang object methods

-- load a dict table to the clientlang dict
function Clientlang:load(dict)
    if dict then
        Clientluang.inject(self.dict, dict)
    end
end

-- load a dict table to the clientlang dict for a specific locale
function Clientlang:loadLocale(locale, dict)
    self:load({[locale] = dict})
end

-- construct Clientlang object
setmetatable(Clientluang, { __call = function(t)
    local obj = {}
    obj.dict = {}
    obj.clientlang = setmetatable({}, { __index = function(t,k) return resolve_path(obj.dict,"",t,k) end })
    return setmetatable(obj, { __index = Clientlang })
end})

-- inject recursively the itable (insert table) properties into the btable (base table)
function Clientluang.inject(btable, itable)
    if type(itable) == "table" then
        for k,v in pairs(itable) do
            local bv = btable[k]
            if type(bv) == "table" then
                Clientluang.inject(bv, v) -- recursive, don't replace table
            else
