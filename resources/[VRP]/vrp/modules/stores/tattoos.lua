...
function vRP.openTattooshop(source, shop)
    ...
    local kitems = {}
    local old_custom = vRPclient.getTattoos(source)
    ...
    local tattoshop_choice = function(player,choice)
        ...
        if tattoo then
            ...
            if tattoo == "CLEAR" then
                ...
                vRPclient.cleanPlayer(source)
            else
                local custom = vRPclient.getTattoos(source)
                ...
                if not applied then
                    ...
                    vRPclient.addTattoo(source, tattoo, shop, price)
                else
                    vRPclient.notify(source,lang.tattoos.removed())
                    vRPclient.delTattoo(source,tattoo)
                end
            end
        end
    end
    ...
    menudata.onclose = function(player)
        ...
        if vRP.tryPayment(user_id,price) then
            ...
            -- revert changes
            vRPclient.setTattoos(source,old_custom)
        end
    end
...
AddEventHandler("vRP:playerSpawn",function(user_id, source, first_spawn)
    if first_spawn then
        ...
        if data then
            ...
            vRPclient.setTattoos(source,tattoos)
        end
    end
end)