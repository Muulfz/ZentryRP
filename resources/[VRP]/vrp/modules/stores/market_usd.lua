...
local cfg = module("cfg/stores/markets_usd")
local market_types = cfg.market_types
local markets = cfg.markets
local market_usd_menus = {}


-- build market menus
local function build_market_menus_usd()
  local config = module("cfg/stores/markets_usd")
  local market_types_ = config.market_types
  local icms = vRP.getICMS()
  local tax = vRP.getIPI()
  local impost = vRP.getII()
  for gtype,mitems in pairs(market_types_) do
    ...
    market_usd_menus[gtype] = market_menu
  end
end

local first_build = true

local function build_client_markets_usd(source)
  -- prebuild the market menu once (all items should be defined now)
  if first_build then
    build_market_menus_usd()
    first_build = false
  end
  ...
  if user_id ~= nil then
    for k,v in pairs(markets) do
      ...
      local menu = market_usd_menus[gtype]
...
AddEventHandler("vRP:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    build_client_markets_usd(source)
  end
end)
